...
毎日スクリプト実行                       「追加文章」機能に表示用HTMLを登録
（毎日 14:03 JST）                      → ショートコード [addtexts addid=XXXX] が発行される
...
| `.github/workflows/update-spread.yml` | 上記スクリプトを毎日14:03(JST)に自動実行するワークフロー |
...
比較表とは別に、**各取引所の数値を単体で**記事本文中に表示できます（例：「GMOコインのスプレッドは0.034%」のように文中に埋め込む）。スタイルは付けていないプレーンなテキスト出力なので、`<strong>`で囲む・CSSを当てるなど記事側で自由に整形できます。

仕組みは「**ローダー方式**」です：追加文章にはスクリプトだけを登録し（不可視・**登録は1つでよい**）、表示したい数値のspanは記事本文に直接書きます。

### 使い方

1. **【初回のみ】** `wordpress/addtexts-value.html` の中身をWPの「追加文章」に貼り付けて保存し、ショートコードを控える（1つ登録すれば全記事で使い回せる）
2. 記事本文の表示したい箇所に、以下の形式のspanを直接書く（何個でも可）：

```html
<p>GMOコインのスプレッドは<span class="cp-spread-value" data-key="gmo" data-field="spread">—</span>です。</p>
```

| 属性 | 指定できる値 |
|---|---|
| `data-key` | `gmo`（GMOコイン）/ `bitflyer` / `coincheck` / `bitbank` |
| `data-field` | `spread`（7日間平均スプレッド率 → 例 `0.021%`）/ `ask`（購入価格 → 例 `10,248,187円`）/ `bid`（売却価格） |

3. 同じ記事のどこか1箇所（位置は任意・末尾でよい）に、控えたローダーのショートコードを挿入する

> ローダーのショートコード自体は**画面に何も表示しません**。記事本文のspanをまとめて数値に置き換えるだけです。

### 補足

- 数値は比較表と同じ `spread.json` から取得され、毎日14時に自動更新されます
- **ローダーの挿入を忘れた記事・取得失敗時は「—」のまま**表示され、記事レイアウトは壊れません（表示が「—」のままの場合はまずローダーのショートコードが記事に入っているか確認）
- 1記事に何個埋め込んでも、データ取得は1回だけ共有されます
...
**基本は放置でOKです。** 毎日14:03(JST)に自動更新されます。

月1回程度、以下を目視確認してください：

- ブラウザで `https://rk-cp-dev.github.io/spread-table/spread.json` を開き、`updated` の日付が当日または前日になっているか
...
- **表に表示するスプレッド率は直近7日間（土日含む）の平均**。spread.json内の`history`に直近7日分（日付・ask・bid・スプレッド率）を保持し、毎回のスプレッド率はその平均値（単一ファイル上書き方式のまま。別ファイル・外部DBは使わない）
  - 蓄積が7日に満たない間は、取得できた日数分の平均（`days`フィールドで日数を確認可能）
