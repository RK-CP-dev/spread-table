# 取引所スプレッド率テーブル（週1自動更新）

CoinPostの「取引所ランキング記事」に掲載する、BTC/JPYスプレッド率テーブルの自動更新システムです。

取引所4社（GMOコイン・bitFlyer・Coincheck・bitbank）の公開APIから週1回自動でスプレッド率を集計し、WordPress記事内の表を最新に保ちます。
...
週1回スクリプト実行                     「追加文章」機能に表示用HTMLを登録
（毎週月曜 09:23 JST）                  → ショートコード [addtexts addid=XXXX] が発行される
...
| `.github/workflows/update-spread.yml` | 上記スクリプトを毎週月曜09:23(JST)に自動実行するワークフロー |
...
**基本は放置でOKです。** 毎週月曜09:23(JST)に自動更新されます。

月1回程度、以下を目視確認してください：

- ブラウザで `https://rk-cp-dev.github.io/spread-table/spread.json` を開き、`updated` の日付が直近の月曜になっているか
...
- サンプル数・間隔は `scripts/collect_spread.py` 冒頭の定数で調整可能
