...
- **表に表示するスプレッド率は直近7営業日の平均**。spread.json内の`history`に直近7営業日分（日付・ask・bid・スプレッド率）を保持し、毎回のスプレッド率はその平均値（単一ファイル上書き方式のまま。別ファイル・外部DBは使わない）
  - 蓄積が7営業日に満たない間は、取得できた日数分の平均（`days`フィールドで日数を確認可能）
